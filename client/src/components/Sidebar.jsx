...
const {logout, OnlineUsers: rawOnlineUsers} = useContext(AuthContext)
  ...
  const fileteredUsers = input ? users.filter((user)=>user.fullName.toLowerCase().
...
console.log("Fetched users from context:", users);
      ...
      <div className='pb-5'>
        ...
        <div className='flex flex-col'>
            {fileteredUsers.map((user, index)=>(
                <div onClick={()=> {setSelectedUser(user); setSelectedUser(prev=>
                ({...prev, [user._id]:0}))}} 
                key={index} className={`relative flex item-centre gap-2 p-2 pl-4 rounded
